...
class Constants {
	static TARGET = document.querySelector('svg#map');
	static TARGET_W = Constants.TARGET.width.baseVal.value;
	static TARGET_H = Constants.TARGET.height.baseVal.value;
	static AUTH = 4447;
	static SILO_W = 4;		// Width of silos
	static SILO_H = 4;		// Height of silos
	static DEFENCE_W = 4;	// Width of defence posts
	static DEFENCE_H = 4;	// Height of defence posts
	static TARGET_BB = Constants.TARGET.getBoundingClientRect();
	static TARGET_OFFSET_X = Constants.TARGET_BB.left.toFixed(2);
	static TARGET_OFFSET_Y = Constants.TARGET_BB.top.toFixed(2);
	static ADD_INCOME = true;
	static TARGET_RADIUS = 2;
	static WEAPON_LAUNCHED_TEXT = 'EN ROUTE';		// Text to show in countdown clock when weapon is launched
	static DO_FOCUS_NEEDED = false;		// Should we check if bother users' windows have focus?

	static SYMBOL_MORE = '<big>&#187;</big>';	// like a '>>' symbol
}
